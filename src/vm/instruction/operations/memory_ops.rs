...
impl Operations {
    ...
    pub fn load_u64(vm: &mut VM) {
        ...
        let result_reg = ol[1] as usize;
        let offset = vm.next_operand_imm();
        unsafe {
            let r = vm.st.r.as_mut_ptr();
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let v = heep_ptr.add(offset as usize);
            *r.add(result_reg) = *(v as *const u64);
        }
        vm.next_step();
    }
    ...
    pub fn load_u32(vm: &mut VM) {
        ...
        unsafe {
            let r = vm.st.r.as_mut_ptr();
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *r.add(result_reg) = *(ptr as *const u32) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn load_u16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *r.add(result_reg) = *(ptr as *const u16) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn load_u8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *r.add(result_reg) = *(ptr as *const u8) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn store_u64(vm: &mut VM) {
        let ol = vm.next_operand();
        let ptr_reg = ol[0] as usize;
        let src_reg = ol[1] as usize;
        let offset = vm.next_operand_imm();
        unsafe {
            let r = vm.st.r.as_mut_ptr();
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *(ptr as *mut u64) = *r.add(src_reg);
        }
        vm.next_step();
    }
    ...
    pub fn store_u32(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *(ptr as *mut u32) = *r.add(src_reg) as u32;
        }
        vm.next_step();
    }
    ...
    pub fn store_u16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *(ptr as *mut u16) = *r.add(src_reg) as u16;
        }
        vm.next_step();
    }
    ...
    pub fn store_u8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heep_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heep_ptr.add(offset as usize);
            *(ptr as *mut u8) = *r.add(src_reg) as u8;
        }
        vm.next_step();
    }
    ...
    pub fn atomic_add_u64(vm: &mut VM) {
        ...
        unsafe {
            let r = vm.st.r.as_mut_ptr();
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            let atomic_ptr = ptr as *mut AtomicU64;
    ...
    pub fn atomic_load_u16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            let atomic_ptr = ptr as *const AtomicU16;
            *r.add(result_reg) = (*atomic_ptr).load(Ordering::SeqCst) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn atomic_store_u16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            let atomic_ptr = ptr as *mut AtomicU16;
    ...
    pub fn atomic_load_u8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            let atomic_ptr = ptr as *const AtomicU8;
            *r.add(result_reg) = (*atomic_ptr).load(Ordering::SeqCst) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn atomic_store_u8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            let atomic_ptr = ptr as *mut AtomicU8;
    ...
    pub fn load_i8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *r.add(result_reg) = (*(ptr as *const i8) as i64) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn load_i16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *r.add(result_reg) = (*(ptr as *const i16) as i64) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn load_i32(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *r.add(result_reg) = (*(ptr as *const i32) as i64) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn load_i64(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *r.add(result_reg) = (*(ptr as *const i64) as i64) as u64;
        }
        vm.next_step();
    }
    ...
    pub fn store_i8(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *(ptr as *mut i8) = *r.add(src_reg) as i8;
        }
        vm.next_step();
    }
    ...
    pub fn store_i16(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *(ptr as *mut i16) = *r.add(result_reg) as i16;
        }
        vm.next_step();
    }
    ...
    pub fn store_i32(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *(ptr as *mut i32) = *r.add(result_reg) as i32;
        }
        vm.next_step();
    }
    ...
    pub fn store_i64(vm: &mut VM) {
        ...
        unsafe {
            ...
            let heap_ptr = vm.st.mem.as_ptr(VPtr(*r.add(ptr_reg)));
            let ptr = heap_ptr.add(offset as usize);
            *(ptr as *mut i64) = *r.add(result_reg) as i64;
        }
        vm.next_step();
    }
